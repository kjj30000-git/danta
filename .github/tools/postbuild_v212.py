...
def verify_opening_observation_and_grace(ns: dict, tmp: pathlib.Path) -> list[str]:
    ...
    try:
        ...
        obs_path = written[-1]
        if not obs_path.name.endswith('_v212.csv') or obs_path.stat().st_size <= 0:
...
def verify_first75_lineage(ns: dict) -> list[str]:
    """FIRST/LATER 독립 gate와 CALM의 확정 FIRST 부모관계를 실제 함수 호출로 검증합니다."""
    moment = datetime(2026, 9, 16, 9, 5, 0)
    later = datetime(2026, 9, 16, 9, 5, 30)
    opened = []
    decisions = []

    ns['has_paper_entered_today'] = lambda *a, **k: False
    ns['attach_session_decision_metrics'] = lambda *a, **k: (True, 'OK')
    ns['attach_pre_first_75_metrics'] = lambda *a, **k: None
    ns['attach_entry_cost_metrics'] = lambda *a, **k: None
    ns['save_entry_decision'] = lambda stock, mode, decision, reason='', extra=None: decisions.append(
        (mode, decision, reason, dict(extra or {}))
    )
    ns['maybe_open_live_trade'] = lambda *a, **k: (_ for _ in ()).throw(
        AssertionError('live order path must not run in RESEARCH')
    )
    ns['LIVE_ENTRY_MODE'] = '__POSTBUILD_NO_LIVE__'

    def open_trade(stock, mode, meta=None, exit_strategies=None, **kwargs):
        tid = f'PB-{mode}-{len(opened) + 1}'
        opened.append((tid, mode, dict(meta or {})))
        ns['paper_positions'][tid] = {}
        return tid

    ns['open_paper_trade'] = open_trade

    def stock_at(code, when):
        stock = base_stock(code)
        stock.update({
            'score_time': when,
            'decision_time': when,
            '_score_evaluated_ts': when.timestamp(),
            '_scan_sample_ts': when.timestamp(),
            'price_change_30s': 0.30,
            'price_change_60s': 1.00,
            'high_gap_change_60s': 0.00,
        })
        return stock

    state = {'first_75_time': moment, 'first_75_pre_status': 'FAIL'}
    ns['register_pre_first_75_if_needed'] = lambda stock, when: state

    ns['FIRST_75_RESEARCH_ENABLED'] = True
    ns['LATER_PASS_RESEARCH_ENABLED'] = False
    ns['evaluate_pre_history'] = lambda stock: ('PASS', 'PASS_FIXTURE')
    first_stock = stock_at('999941', moment)
    first_tid = ns['maybe_open_pre_history'](first_stock, moment)
    if not first_tid or first_stock.get('pre_entry_type') != 'FIRST_75_PASS':
        raise RuntimeError('case A: FIRST_75_PASS was not created')

    opened.clear()
    fail_stock = stock_at('999942', moment)
    ns['evaluate_pre_history'] = lambda stock: ('FAIL', 'FAIL_FIXTURE')
    if ns['maybe_open_pre_history'](fail_stock, moment) is not None or opened:
        raise RuntimeError('case B: PRE FAIL created FIRST_75_PASS')

    opened.clear()
    ns['evaluate_pre_history'] = lambda stock: ('PASS', 'PASS_FIXTURE')
    later_off_stock = stock_at('999943', later)
    if ns['maybe_open_pre_history'](later_off_stock, later) is not None or opened:
        raise RuntimeError('case C: LATER OFF created a paper trade')
    if later_off_stock.get('pre_entry_type') != 'LATER_PASS':
        raise RuntimeError('case C: LATER identity was not preserved')

    opened.clear()
    ns['LATER_PASS_RESEARCH_ENABLED'] = True
    later_on_stock = stock_at('999944', later)
    later_tid = ns['maybe_open_pre_history'](later_on_stock, later)
    if not later_tid or later_on_stock.get('pre_entry_type') != 'LATER_PASS':
        raise RuntimeError('case D: LATER ON did not create LATER_PASS')

    opened.clear()
    ns['pre_first_75_states']['999945'] = dict(state)
    calm_fail_stock = stock_at('999945', moment)
    calm_fail_stock['pre_entry_type'] = ''
    if ns['maybe_open_calm_first_75'](calm_fail_stock, None) or opened:
        raise RuntimeError('case E: CALM opened without confirmed FIRST parent')

    opened.clear()
    ns['pre_first_75_states']['999946'] = dict(state)
    calm_stock = stock_at('999946', moment)
    calm_stock['pre_entry_type'] = 'FIRST_75_PASS'
    calm_ids = ns['maybe_open_calm_first_75'](calm_stock, 'PB-PRE-FIRST')
    modes = [mode for _, mode, _ in opened]
    if modes != ['CALM_FIRST_75', 'CALM_FIRST_75_PROTECT'] or len(calm_ids) != 2:
        raise RuntimeError(f'cases F/G: CALM lineage mismatch: {modes}')
    if any(meta.get('parent_pre_trade_id') != 'PB-PRE-FIRST' for _, _, meta in opened):
        raise RuntimeError('case H: CALM parent trade id was not stored')
    if not ns['paper_positions'][calm_ids[1]].get('profit_protect_enabled'):
        raise RuntimeError('case G: CALM protect flag missing')

    return [
        'A/B: FIRST PASS 1건 / FIRST PRE FAIL 0건',
        'C/D: LATER OFF 0건 / LATER ON 1건, pre_entry_type 보존',
        'E/F/G/H: CALM은 확정 FIRST 부모의 부분집합이며 부모 trade_id 저장',
    ]


def first75_negative_controls(target: pathlib.Path, tmp: pathlib.Path) -> list[str]:
    nb = load_nb(target)
    cases = [
        (
            'later_gate',
            'or (pre_entry_type == "LATER_PASS" and LATER_PASS_RESEARCH_ENABLED)',
            'or (pre_entry_type == "LATER_PASS")',
        ),
        (
            'calm_parent',
            'passed = (is_confirmed_first and available and CALM_30S_MIN_PCT',
            'passed = (is_first and available and CALM_30S_MIN_PCT',
        ),
    ]
    results = []
    for label, old, new in cases:
        ...
        path = tmp / f'negative_{label}.ipynb'
        path.write_text(json.dumps(bad, ensure_ascii=False, indent=1) + '\n', encoding='utf-8')
        bad_ns = execute_release(path, tmp)
        try:
            verify_first75_lineage(bad_ns)
        except RuntimeError:
            results.append(f'{label}: behavior regression FAIL as expected')
        else:
            raise RuntimeError(f'negative control {label}: behavior test unexpectedly passed')
    return results
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td_raw:
        ...
        first75 = verify_first75_lineage(ns)
        first75_negative = first75_negative_controls(args.target, td)
    ...
    lines = [
        ...
        *[f'- PASS: {x}' for x in observation],
        ...
        '## FIRST_75 계통 행동 회귀',
        '',
        *[f'- PASS: {x}' for x in first75],
        *[f'- PASS: {x}' for x in first75_negative],
        '',
        '## 판정',
        '',
